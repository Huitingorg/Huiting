...
, { -10256, "zun" }, { -10254, "zuo" } };
        ...
        public static string ToPinYin(string txt, string Separator)
        {
            txt = txt.Trim();
                ...
                else
                {
                    ...
                    if (!CodeCollections.ContainsKey(charCode))
                    {
                        for (int i = charCode; i <= 0; --i)
                        {
                            ...
                            {
                                result.Append(Separator + CodeCollections[i] + Separator);
                                break;
                            }
                        }
                    }
                    ...
                    {
                        result.Append(Separator + CodeCollections[charCode] + Separator);
                    }
                }
            }
        ...
        public static string ToPinYin(string txt, bool FirstWordUp)
        {
            if (txt == null)
                return "";
            ...
            for (int j = 0; j < arrChar.Length; j++)   //遍历输入的字符 
            {
                ...
                else
                {
                    ...
                    {
                        string pinYin = CodeCollections[charCode];
                        TextInfo tInfo = Thread.CurrentThread.CurrentCulture.TextInfo;
                        //TextInfo tInfo = new CultureInfo("en-US", false).TextInfo;
                        pinYin = tInfo.ToTitleCase(pinYin);
                        result.Append(pinYin);
		...
		public static string GetPYFirstWord(string strInfo)
		{
			StringBuilder sb = new StringBuilder();
			foreach (char item in strInfo)
			{
				char c1 = GetPYFirstWord(item);
				sb.Append(c1);
			}
		...
		public static char GetPYFirstWord(char c)
		{
			byte[] array = new byte[2];

			array = System.Text.Encoding.Default.GetBytes(c.ToString());
			if (array.Length <= 0)
				return '*';
