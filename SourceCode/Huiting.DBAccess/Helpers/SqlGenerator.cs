...
namespace Huiting.DBAccess.Helpers
{
    ...
    public static class SqlGenerator
    {
        ...
        /// <param name="type"></param>
        /// <returns></returns>
        public static string GetInsertSqlByModel(Type type, bool withReturnID = false)
        {
            StringBuilder sb1 = new StringBuilder();
            StringBuilder sb2 = new StringBuilder();
            //var tableObj = (DataTableAttribute[])type.GetCustomAttributes(typeof(DataTableAttribute), false);
            ...
            string tableName = DapperHelper.TableNames[type];
            string sql = $"INSERT INTO {tableName} ({sb1.ToString().TrimEnd(',')}) VALUES ({sb2.ToString().TrimEnd(',')})";
            if (withReturnID && string.IsNullOrWhiteSpace(sql) == false)
