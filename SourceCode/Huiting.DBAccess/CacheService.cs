...
namespace Huiting.DBAccess
{
    public class CacheService
    {
        ...
        private void AddTableCache<T>(SQLiteConnection conn, SQLiteTransaction trans) where T : IEntity
        {
            ...
            //MemoryCacheHelper.AddTable(tableKey, DBTableGenerator.Instance.Logics[tableKey]?.GetEntitys<T>(conn, trans),
            //    o => DBTableGenerator.Instance.Logics[tableKey]?.GetEntityKey(o));
        }
        ...
        public void Update<T>(string tableKey, List<T> data) where T : IEntity
        {
            try
            {
               // MemoryCacheHelper.Update(tableKey, data, o => DBTableCorrector.Instance.Logics[tableKey]?.GetEntityKey(o));
            }
            catch (Exception ex)
            {
